...
import { mileageGetHistory, mileageGetHistoryRes, mileageUsingHistory, mileageUsingHistoryRes } from '@/types/myPage';
...
export default function MileageHistory() {
  ...
  const [usageHistory, setUsageHistory] = useState<mileageUsingHistory[]|null>(null);
  const [rewardHistory, setRewardHistory] = useState<mileageGetHistory[] | null>(null);
  ...
  const MileageGetHistory = async () => {
    try {
      const response = await getAPI<mileageGetHistoryRes>('/api/mypage/mileage-gethistory');
      ...
      <div className='w-[988px] flex justify-between items-center'>
        ...
        <div className='flex'>
          <div className='flex items-center h-[22px]'>
              <p className={`w-[47px] border-r-2 border-blue text-[18px] cursor-pointer ${activeTab === 'usage' ? 'text-blue' : ''}`}
                onClick={() => {
                  setActiveTab('usage');
                }}
              >
              ...
              <p className={`flex-grow text-[18px] cursor-pointer text-left pl-[10px] ${activeTab === 'reward' ? 'text-blue' : ''}`}
                onClick={() => {
                  setActiveTab('reward')
                }}
              >
